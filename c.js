// Write your Javascript code.
		...
		var insts = new Array(
			...
			"POW","ATN2","FABS","ATAN","LOG","LOGT","EXP","FLOR","CEIL","HYPO","SIN","COS","TAN","ASIN","ACOS","SINH","COSH","TANH","SQRT","FMOD"
		);
		var i_id = {};
	  ...
	  = op_st[op_t["/="]] = op_st[op_t["|="]] = op_st[op_t["<<="]] = op_st[op_t[">>="]] = "BINARY";
	  	op_st["NUMBER"] = op_st["PNUMBER"] = "BINARY";
	  	op_st["IF"] = "IF";
	  	op_st["ELSE"] = "ELSE";
	  	
	  	op_r["OFFSET"] = op_r["NUMBER"]= op_r["PNUMBER"] = 1;
	  ...
	  = op_r[op_t["/="]] = op_r[op_t["|="]] = op_r[op_t["<<="]] = op_r[op_t[">>="]] = 14;
	  	op_r["NEXT"] = 15;
			...
			switch (type) {
				...
				case "stmt" :
					...
					switch (word) {
						...
						case "asm" :
							...
							i = this.spaceskip(j+1); j = i;
							while (/[0-9]/.test(this.text.slice(j, j+1))) j++;
							et.imme = parseInt(this.text.slice(i, j));
							switch (this.text[j]) {
				...
				case "expr" :
					...
					while (true) {
						...
						*/
						if (this.text[j] == '.' || this.text.slice(j, j+1) == "->") break;
						if (this.isEndCode(this.text[j], "expr")) break;
						et = new Object();
						switch (this.text[j]) {
							case '(' :
					...
					if (this.text[j] == '.') {
						...
						et.type = "number";
						et.struct = tmp;
						println("s_type="+tmp.s_type);
						et.number = this.analysis(j+1, tmp.s_type+"::", "expr");
						et.end = et.number.end;
					}
					if (this.text.slice(j, j+1) == "->") {
						g_ana.analysis(et, prev);
						if (et.s_type == "error") {
							et.type = "error";
							return et;
						...
						et.type = "pnumber";
						et.struct = tmp;
						et.number = this.analysis(j+2, tmp.s_type.slice(0, tmp.s_type.length-1)+"::", "expr");
						et.end = et.number.end;
					}
					if (this.text[et.end] == ',') {
				...
				case "s_code" :
					...
					switch (word) {
						...
						case "case" :
							et.type = "case";
							et.value = this.analysis(j, prev, "const");
							if (et.value.type == "error")
								return et.value;
							et.end = this.spaceskip(et.value.end+1);
				...
				case "enum" :
					...
					this.rec.setVariable(et, prev);
					if (this.text[et.end] == '=') {
						et.value = this.analysis(et.end+1, prev, "expr");
						if (et.value.type == "error")
							return et.value;
						et.end = et.value.end;
					}
		...
		g_ana.analysis = function(et, prev) {
			println("analysis type="+et.type+" prev="+prev+" op_st="+et.op_st+" start");
					 ...
					 case "BINARY" : 
					 	...
					 	if (this.rec.isStructtype(et.value1.s_type) ||
					 		...
					 		if (et.value1.s_type[et.value1.s_type.length-1] == '*' &&
								...
								if (et.value1.s_type != "int")
									et.s_type = et.value1.s_type;
								if (et.value2.s_type != "int")
									et.s_type = et.value2.s_type;
								switch (et.type.slice(5)) {
									case "ADD" : case "SUB":
										break;
									case "ASSIGNADD" : case "ASSIGNSUB" :
										if (et.value2.s_type != "int")
											this.error(et.end, "invalid operands to binary "+op_rt[et.type.slice(5)]+
					 						" (have '"+et.value1.s_type+"' and '"+et.value2.s_type+"')");
					 					et.s_type = et.value1.s_type;
					 		...
					 		et.s_type = "bool";
					 		if (et.value1.s_type != "bool" || et.value2.s_type != "bool") 
					 			this.error(et.end, "invalid operands to binary "+op_rt[et.type.slice(5)]+
					 			" (have '"+et.value1.s_type+"' and '"+et.value2.s_type+"')");
						} else {
					...
					default :
						switch (et.type.slice(5)) {
							case "VAR" : 
								...
								println("name="+et.value.name+" s_type="+et.value.s_type);	
								et.s_type = et.value.s_type; 
								println("type=var b2"); 
								break;
							case "FUNC" : 
			...
			switch (et.type) {
				...
				case "func" :
					...
					//println("func name="+et.name+" stack_sz="+this.rec.stack_sz[s_prev]+" "+this.rec.getStackSize(s_prev));
					for (e = et.reg ; e != null && e.type != "empty" ; e = e.next)
				...
				case "expr" :
					...
					for (var i = 0 ; i < et.text.length ; i++) {
						...
						else if (et.text[i].type == "CONV" && (i==et.text.length-1 || op_st[et.text[i+1].type] != null)) {
							et.text[i].type = "VAR";
				...
				case "switch" :
					...
					break;
				case "case" :
					et.value.type = "expr_"+et.value.type;
				...
				case "continue" :
					loop = this.rec.loops[prev];
					if (loop == null) {
						this.error(et.end, "continue statement not within a loop"); break;
					}
					if (loop.type == "for")
				...
				case "asm" :
					if (i_id[et.inst] == null)
						this.error(et.end, "undifined instruction '"+et.inst+"'");
					break;
				default :
					break;
			}
